...
x=cx+radius*cos(theta*3.14/180);
y=cy+radius*sin(theta*3.14/180);
/** Draw Numeric Points **/
circle(x,y,2);
...
do{
putpixel(cx+radius*cos(i*3.14/180),cy+radius*sin(i*3.14/180),DARKGRAY);
i+=6;
}while(i!=360);
...
float sx,sy;
setcolor(0);
sx=cx+S_N_L*cos((angle+s*6-6)*3.14/180);
sy=cy+S_N_L*sin((angle+s*6-6)*3.14/180);
line(cx,cy,sx,sy);
setcolor(S_N_C);
sx=cx+S_N_L*cos((angle+s*6)*3.14/180);
sy=cy+S_N_L*sin((angle+s*6)*3.14180);
line(cx,cy,sx,sy);
}
